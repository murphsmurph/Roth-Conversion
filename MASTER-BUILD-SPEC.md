...
- **`verifiedBy: PENDING_ADVISOR_REVIEW`** on every file — a human advisor sign-off is a **required
  gate** before client use and has not happened. The sign-off packet
  (`docs/ADVISOR-REVIEW-PACKET.md`, regenerated by `npm run build:review` from the rules JSON so the
  values reviewed are exactly the ones the engine runs on) lists every constant with authority,
  source, and a tick box for a CPA/EA to review and attest; on sign-off, set `verifiedBy` and
  regenerate.
